...
namespace Parabox.CSG
{
    sealed class Node
    {
        ...
        public int MaxPolygonsPerNode = 1;
        ...
        public void Build(List<Polygon> list, int depth = 0)
        {
            if (list == null || list.Count == 0)
                return;

            if (depth > MaxRecursionDepth || list.Count <= MaxPolygonsPerNode)
            {
                polygons = new List<Polygon>(list);
                return;
            }
            ...
            {                
                ...
                if (newNode && list.SequenceEqual(listFront))
                    ...
                    (front ?? (front = new Node())).Build(listFront, depth + 1);
            }
            ...
            if (listBack.Count > 0)
            {
                if (newNode && list.SequenceEqual(listBack))
                    ...
                    (back ?? (back = new Node())).Build(listBack, depth + 1);
            }
        }
        ...
        {
            if (this.plane == null || this.plane.Valid() == false)
            {
                return list;
            }
