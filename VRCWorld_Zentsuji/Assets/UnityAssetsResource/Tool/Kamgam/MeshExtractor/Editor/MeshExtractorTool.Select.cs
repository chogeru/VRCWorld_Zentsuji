...
namespace Kamgam.MeshExtractor
{
    partial class MeshExtractorTool
    {
        ...
        public void DefragSelectedObjects()
        {
            // Clear out deleted selections
            ...
            _selectedObjects = newSelection.ToArray();
        }
        ...
        protected void onSelectGUI(SceneView sceneView)
        {
            ...
            {
                _undoStack.Add(new UndoState(new HashSet<SelectedTriangle>()), 0f, true);
            }
            ...
            if ((evt.isMouse || evt.isScrollWheel || evt.isKey || evt.type == EventType.Repaint) && !evt.alt)
            {
                ...
                if (isMouseInSceneView())
                {
                    ...
                    if (_hoverTriangles.Count > 0)
                    {
                        ...
                        if (_leftMouseIsDown && !Tools.viewToolActive && _mode == Mode.PaintSelection)
                        {
                            recordSelectionUndoIfEmpty();
                    ...
                    {
                        if (_mode == Mode.PaintSelection && _selectedObjects.Length > 0 && IsMouseInSceneView())
                        {
                            var lastUndoStep = _undoStack.Peek();
                            bool selectionDidChange =
                                (    (lastUndoStep != null && lastUndoStep.Type != UndoStateType.Selection)
                                  || (lastUndoStep != null && lastUndoStep.SelectionData != null && _selectedTriangles.Count != lastUndoStep.SelectionData.Count)
                                )
                                || _selectedTrianglesChangedSinceLastUndoRecording;
                            if (selectionDidChange)
                            {
                                _selectedTrianglesChangedSinceLastUndoRecording = false;
                                _undoStack.Add(new UndoState(_selectedTriangles), 0.2f);
                            }
                        }
                    }
                }
                ...
                if(_mode == Mode.PaintSelection && _saveLoadVisibility == 0 
                    && evt.isKey && evt.modifiers == EventModifiers.None && evt.keyCode == MeshExtractorSettings.GetOrCreateSettings().TriggerSelectLinked
        ...
        void recordSelectionUndoIfEmpty()
        {
            if (!_undoStack.HasUndoActions())
            {
                _selectedTrianglesChangedSinceLastUndoRecording = false;
                _undoStack.Add(new UndoState(_selectedTriangles), 0f, forceNewGroup: true);
            }
        }
        ...
        void addSameMaterialToSelection(bool remove = false)
        {
            recordSelectionUndoIfEmpty();
            ...
            var settings = MeshExtractorSettings.GetOrCreateSettings();
            var sameMaterialTris = TriangleCache.AddSameMaterial(_selectedObjects, _lastClickedTriangle);
            foreach (var tri in sameMaterialTris)
            {
                if (remove)
                {
                    if (_selectedTriangles.Contains(tri))
        ...
        public void onSelectRenderMesh()
        {
            ...
            if (_mode == Mode.PaintSelection && _hoverTriangles.Count > 0 && !_mouseIsDown)
            {
        ...
        protected void loadSelection(SelectedTriangleSnapshot snapshot, bool additive)
        {
            TriangleCache.RebuildBakedMeshesCache(_selectedObjects);
