    ...
    VertexAttributes attributes)
        {
            if (vertices == null)
            {
                Debug.LogError("GetArrays: vertices list is null.");
                position = null;
                color = null;
                uv0 = null;
                normal = null;
        ...
        public static void SetMesh(Mesh mesh, IList<Vertex> vertices)
        {
            ...
            if (vertices.Count == 0)
            {
                Debug.LogError("SetMesh: No vertices provided.");
                return;
            }
            ...
            if (first.hasUV3 && uv3s != null && uv3s.Count > 0) mesh.SetUVs(2, uv3s);
            if (first.hasUV4 && uv4s != null && uv4s.Count > 0) mesh.SetUVs(3, uv4s);
        }
